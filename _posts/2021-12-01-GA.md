...
If I have created a new page on my application and pushed the code into github, the above workflow will scan the target “your-website.com”, but since the new code is not yet deployed on “your-website.com” yet, what really is the scan doing? I am sure you guessed it, it is scanning what is already deployed.
...
This is where we install OWASP zap from scratch, I don’t like using the one on the marketplace because I have experience some networking issues while running runners parallelly.
...
```
This is where I am doing some manual intervention, this “zap.py” script will essentially run a passive and active scan on our target application and generate a report. 

You can see the zap.py here : [zap.py](https://github.com/san3ncrypt3d/sampleapp/blob/master/script/zap.py)

Why is this script a good idea? Because till now we have just been attacking apps without any authentication and authorization, so what if the page added is inside an authenticated portal? We can modify this script to do authentication and spider pages.
